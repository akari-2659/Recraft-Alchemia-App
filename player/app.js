(()=>{
  ...
  async function refreshBindingInBackground(session,cached){
    try{const result=await ACCOUNT.getBinding(session.loginPlayerKey);if(!result.found||!result.binding){location.replace('../');return;}const oldChar=String(window.RA_PLAYER_CONTEXT.charKey||''),oldFacility=String(window.RA_PLAYER_CONTEXT.facilityKey||'');applyBindingContext(result.binding,{...session,role:result.role||session.role});if(oldChar!==window.RA_PLAYER_CONTEXT.charKey){characters=[];selectedCharacterId='';loadCharacterList({background:false});}if(oldFacility!==window.RA_PLAYER_CONTEXT.facilityKey&&moduleState.facility.ready)reloadFrame('facility');}
  ...
  async function bootstrapAccount(){
    ...
    try{const result=await ACCOUNT.getBinding(session.loginPlayerKey);if(!result.found||!result.binding){location.replace('../');return false;}applyBindingContext(result.binding,{...session,role:result.role||session.role});return true;}catch(error){location.replace('../');return false;}
  }
